...
#   @micropython.native
  ...
  def zhtext(self, aPos, aString, aColor, aFont, aSize=1, nowrap=False):
    '''专用于绘制 16x16 汉字的文本'''
    if aFont is None:
        return
    ...
    width = wh[0] * aFont["Width"] + 1
    for c in aString:
        self.zhchar((px, py), c, aColor, aFont, wh)
        px += width
        if px + width > self._size[0]:
            if nowrap:
                break
                ...
                py += aFont["Height"] * wh[1] + 1
                px = aPos[0]
  def zhchar(self, aPos, aChar, aColor, aFont, aSizes):
    '''专用于绘制 16x16 点阵汉字（横向排列）'''
    if aFont is None:
        return
    if aChar not in aFont["Data"]:
        return

    fontw = aFont["Width"]
    fonth = aFont["Height"]
    bitmap = aFont["Data"][aChar]
    scalex, scaley = aSizes
    px0, py0 = aPos
    ...
    for y in range(fonth):
        ...
        for x in range(fontw):
            if (row >> (fontw - 1 - x)) & 1:
                self.fillrect((px0 + x * scalex, py0 + y * scaley), (scalex, scaley), aColor)
